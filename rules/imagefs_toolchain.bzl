    ...
    if rctx.attr.tc_os == "qnx":
        ...
        rctx.template(
            ...
            rctx.attr._linux_toolchain_build,
            {
                ...
                "%{e2fsprogs_pkg_repo}": rctx.attr.e2fsprogs_pkg_repo,
                "%{tc_type}": rctx.attr.tc_type,
            },
        )
    else:
...
imagefs_toolchain = repository_rule(
    ...
    attrs = {
        ...
        "coreutils_pkg_repo": attr.string(doc = "Repo providing a ':coreutils' file target (e.g. generated via bazel_lib's register_coreutils_toolchains). Only used when tc_os == 'linux'.", mandatory = False, default = ""),
        "e2fsprogs_pkg_repo": attr.string(doc = "Repo providing the ':mke2fs' file target (e.g. generated via bazel_lib's mke2fs rule). Only used when tc_os == 'linux'.", mandatory = False, default = ""),
